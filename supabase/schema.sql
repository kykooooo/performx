...
begin
  select coalesce(reviews_count, 0), coalesce(rating, 0)
  ...
  update coaches
    set reviews_count = new_count,
        rating = ((current_rating * current_count) + new.rating) / new_count
    where id = new.coach_id;
...
for each row execute procedure update_coach_rating();
